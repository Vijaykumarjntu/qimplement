...
def extract_invoice_data(text, filename):
    ...
    response = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": f"""Extract invoice data from this document. Return ONLY valid JSON (no markdown, no explanations).
...
"""
    ...
    try:
        ...
        cleaned_content = re.sub(r'[^\x00-\x7F]+', '', cleaned_content)  # Remove non-ASCII
        result = json.loads(cleaned_content)
